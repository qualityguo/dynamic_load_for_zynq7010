...
u32 InitSD(const char *filename)
{
	...
	if (rc) {
		fsbl_printf(LOG_DEBUG,"SD: Unable to open file %s: %d\n", boot_file, rc);
		return XST_FAILURE;
	}
...
u32 SDAccess( u32 SourceAddress, u32 DestinationAddress, u32 LengthBytes)
{
	...
	if (rc) {
		fsbl_printf(LOG_DEBUG,"SD: Unable to seek to %lx\n", SourceAddress);
		return XST_FAILURE;
	}
	...
	if (rc) {
		fsbl_printf(LOG_DEBUG,"*** ERROR: f_read returned %d\r\n", rc);
	}
