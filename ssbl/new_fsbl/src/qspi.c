...
u32 InitQspi(void)
{
	...
	if (XPAR_XQSPIPS_0_QSPI_MODE == SINGLE_FLASH_CONNECTION) {
		...
		fsbl_printf(LOG_INFO,"QSPI is in single flash connection\r\n");
		/*
		...
		if (QspiFlashSize <= FLASH_SIZE_16MB) {
			...
			switch (XPAR_XQSPIPS_0_QSPI_BUS_WIDTH) {
				...
				{
					fsbl_printf(LOG_INFO,"QSPI is in 1-bit mode\r\n");
				...
				{
					fsbl_printf(LOG_INFO,"QSPI is in 2-bit mode\r\n");
				...
				{
					fsbl_printf(LOG_INFO,"QSPI is in 4-bit mode\r\n");
	...
	if (XPAR_XQSPIPS_0_QSPI_MODE == DUAL_PARALLEL_CONNECTION) {
		...
		fsbl_printf(LOG_INFO,"QSPI is in Dual Parallel connection\r\n");
		/*
	...
	if (XPAR_XQSPIPS_0_QSPI_MODE == DUAL_STACK_CONNECTION) {
		...
		fsbl_printf(LOG_INFO,"QSPI is in Dual Stack connection\r\n");
...
u32 FlashReadID(void)
{
	...
	fsbl_printf(LOG_INFO,"Single Flash Information\r\n");

	fsbl_printf(LOG_INFO,"FlashID=0x%x 0x%x 0x%x\r\n", ReadBuffer[1],
			ReadBuffer[2],
			ReadBuffer[3]);
...
u32 QspiAccess( u32 SourceAddress, u32 DestinationAddress, u32 LengthBytes)
{
	...
	if (LinearBootDeviceFlag == 1) {
		...
		while(LengthBytes > 0) {
			...
			if (XPAR_XQSPIPS_0_QSPI_MODE == DUAL_STACK_CONNECTION) {
				...
				if (SourceAddress >= (QspiFlashSize/2)) {
					...
					fsbl_printf(LOG_INFO, "stacked - upper CS \n\r");
			...
			if ((SourceAddress >= FLASH_SIZE_16MB) && (BankSwitchFlag == 1)) {
				...
				fsbl_printf(LOG_INFO, "Bank Selection %lu\n\r", BankSel);

				Status = SendBankSelect(BankSel);
				if (Status != XST_SUCCESS) {
					fsbl_printf(LOG_INFO, "Bank Selection Failed\n\r");
					return XST_FAILURE;
				}
		...
		if (Status != XST_SUCCESS) {
			fsbl_printf(LOG_INFO, "Bank Selection Reset Failed\n\r");
			return XST_FAILURE;
		}
		...
		if (XPAR_XQSPIPS_0_QSPI_MODE == DUAL_STACK_CONNECTION) {
			...
			fsbl_printf(LOG_INFO, "stacked - lower CS \n\r");
...
u32 SendBankSelect(u8 BankSel)
{
	...
	if (ReadBuffer[1] != BankSel) {
		fsbl_printf(LOG_INFO, "BankSel %d != Register Read %d\n\r", BankSel,
				ReadBuffer[1]);
		return XST_FAILURE;
	}
