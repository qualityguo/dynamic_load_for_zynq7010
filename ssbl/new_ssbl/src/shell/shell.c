...
#if SHELL_AUTO_PRASE == 0
int shellSetVariableArgv(int argc, char *argv[])
{
    if (argc < 3)
    {
        shellDisplay(shellGetCurrent(), "usage: setVar <name> <value>\r\n");
        return 0;
    }
    char *name = argv[1];
    if (name[0] == '$')
    ...
    shellSetVariable(name, (int)strtoul(argv[2], NULL, 0));
    return 0;
}
SHELL_EXPORT_CMD(setVar, shellSetVariableArgv, setVar <name> <value>);
#else
