...
| Phase 12 | 网络（新增） | M12 | NetXDuo 集成 + Web 配置页 + 网络升级（复用现有 hello_netxduo 移植） |

**实施次序约束**：Phase 0 → 1 → 2 → 3 强制顺序（每级依赖上一级链通）。Phase 4–10 内部多数可并行（依赖 Phase 3 已链通）。Phase 11 是后期独立工作。Phase 12（网络）依赖 Phase 4（storage 层）、Phase 5（image_loader/CRC）、Phase 6（boot_config）已就绪，与 Phase 11 互不依赖，可并行或后做。

> **Note（Phase 12 路径约定）：** 本 Phase 文件路径沿用 spec §3.1 的约定路径 `boot/ssbl/src/`；实际 Vitis 工程落地为 `ssbl/ssbl/src/`（与 ThreadX_A9 / FileX_A9 同级）。两者一一对应，下文不再区分。
...
- **Phase 11**：独立后期工作。
- **Phase 12（网络）**：依赖 Phase 4（storage 层）、Phase 5（image_loader/CRC）、Phase 6（boot_config）已就绪。与 Phase 11 互不依赖，可并行或后做。最大工作量在 HTTP server 集成与 Web 页面；GEM/PHY 驱动与 NetXDuo 核心库复用 `hello_netxduo` 现有移植，无需重写。
...
> **Phase 12 补充**：网络 Phase 是 spec 之外的扩展，已附在 Phase 11 之后。执行时可独立于 M1-M11 主线，待 storage/boot_config/handoff 就绪后再启动。复用 `hello_netxduo` 移植成果，Task 12.1/12.2 以拷贝为主，真正的新代码集中在 Task 12.3-12.6。
